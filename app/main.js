require([
    ...
    .then(function(response) {
      ...
      for (let i = 0; i < cities.length; i++) {
        ...
        button.addEventListener("click", function() {
          ...
          setId(city.id);
          if (city.attribution) {
            document.getElementById("attribution").innerHTML = city.attribution + '. Gemaakt met <a href="" target="_blank">ArcGIS API for JavaScript</a>';
          }
        }.bind(city));
  ...
  document.getElementById("mode").addEventListener("click", function(evt) {
    ...
    if (mode === "light") {
      mode = "dark";
      evt.target.innerHTML = "Potlood";
      document.getElementById("customCSS").href = "./styles/dark.css";
    } else {
      mode = "light";
      evt.target.innerHTML = "Krijt";
